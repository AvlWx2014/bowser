    ...
    def upload(self, source: Path) -> None:
        ...
        for root, _, files in os.walk(source):
            for file in filter(self._filter, files):
                ...
                metadata = get_metadata_for_file(local)
                for_upload.append(_FileMetadataPair(local, metadata))

        for bucket in self._config.buckets:
            for file, metadata in for_upload:
                relative_path = file.relative_to(source.parent)
                # lstrip to remove any unwanted leading "/" e.g. if `bucket.key` is empty
                key = f"{bucket.key}/{relative_path!s}".lstrip("/")
                logging.debug("Uploading %s to %s/%s", file, bucket.name, key)
                self._client.put_object(
                    Body=str(file),
                    Bucket=bucket.name,
                    Key=key,
                    Metadata=metadata,  # TODO: metadata as tags instead of metadata?
