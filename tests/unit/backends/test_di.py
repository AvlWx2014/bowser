...
def test_provide_BowserBackends(region: str, tmp_path: Path):  # noqa: N802
    backend_config = [
        AwsS3BowserBackendConfig(
            region=region,
            access_key_id=SecretStr("testing"),
            secret_access_key=SecretStr("testing"),  # nosec B106
            buckets=[Bucket(name="test-bucket", prefix="")],
        )
    ]
    config = BowserConfig(dry_run=False, backends=backend_config)
    expected_backend_types = [AwsS3Backend]
    with provide_BowserBackends(watch_root=tmp_path, config=config) as actual_backends:
